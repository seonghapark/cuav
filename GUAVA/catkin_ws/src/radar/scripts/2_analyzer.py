...
class ifft_handler():
    ...
    def data_process(self, sync, data):
        ...
        for ii in range(val, int((sync.shape[0] - self.n)), spliter):  # improved searching loop
            ...
            #     continue
            if (ii - spliter > 0) & (sync[ii] == True) & (sync[
                                                          ii - val - spliter:ii - spliter].max() == False):  # if start[ii] is true and the mean of from start[ii-11] to start[ii-1] is zero (All False)
                for jj in range(ii - spliter, ii):
                    if (sync[jj] == True) & (sync[jj - val:jj - 1].mean() == 0.0):
                        self.fsif[count, :] = data[
                                              jj:jj + self.n]  # then copy rightarray from ii to ii+n and paste them to sif[count] --> sif[count] is a list
                        result_time.append((jj + int(sync.shape[
                                                         0]) * self.opp) * 1. / self.fs)  # append time, the time is ii/fs --> few micro seconds (0.0001 sec or so)
                        count = count + 1

                        self.fsif[count, :] = data[
                                              jj + 1:jj + 1 + self.n]  # then copy rightarray from ii to ii+n and paste them to sif[count] --> sif[count] is a list
                        result_time.append((jj + 1 + int(sync.shape[
                                                             0]) * self.opp) * 1. / self.fs)  # append time, the time is ii/fs --> few micro seconds (0.0001 sec or so)
                        count = count + 1
        ...
        zpad = 440
        decibel = self.dbv(
            np.fft.ifft(sif, zpad, 1))  # Do fft calculation, and convert results to decibel through dbv function
